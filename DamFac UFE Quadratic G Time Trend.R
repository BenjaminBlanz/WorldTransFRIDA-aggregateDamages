...
# with Quadratic Global Time Trend
...
# Time Trends ####
# For Linear Global Time Trend activate

# DamFuc$t <- DamFuc$year

# For Quadratic Global Time Trend activate
DamFuc$t <- DamFuc$year
...
plot(sta_vec[ok], damage[ok], pch = 20,
     ...
     main = "Damage Factor versus Tempertaure (UFE) with Quadratic Global Time Trend")
...
plot(xlim, ylim, type = "n",                             # We draw an empty plot, Cartesian plane, first. 
     xlab = "Year",
     ylab = "GDP",
     main = "\nDiagnostics 4: GDP Comparison UFE with Quadratic Global Time Trend",
     cex.main = 1.8)
